            ...
            for cvI in range(0, NumCV):
                ...
                if cvI < NumCV - 1:
                    ...
                    subjectNums_cvI_trainVal = np.delete(subject_nums_shaffled, range(cvI * int(numSubj / NumCV),
                                                                                      cvI * int(numSubj / NumCV) + int(
                                                                                          numSubj / NumCV)))
                else:
                    subjectNums_cvI_testing = subject_nums_shaffled[cvI * int(numSubj / NumCV):int(numSubj * 0.2)]
                    subjectNums_cvI_trainVal = np.delete(subject_nums_shaffled,
                                                         range(int(numSubj * 0.2), numSubj))

                counterI = 0

                # Training CT scans
                for subItrain in range(int(numSubj / NumCV),
                                       len(subjectNums_cvI_trainVal)):  # take only the 3 folds for training
