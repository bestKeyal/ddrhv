...
if __name__ == '__main__':
    ...
    for cvI in range(0, num_CV):
        ...
        if num_CV != 1:
            ...
            subjectNums_cvI_testing = [80, 26]
            subjectNums_cvI_trainVal = [x for x in subject_nums_shaffled if x not in [80, 26]]
    ...
    print(
        "Final pixel-wise testing: mean Jaccard %.3f (max %.3f, min %.3f, +- %.3f),"
        " mean Dice %.3f (max %.3f, min %.3f, +- %.3f)" % (
